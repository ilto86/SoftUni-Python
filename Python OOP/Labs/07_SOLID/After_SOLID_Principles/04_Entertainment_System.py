...
class Television(RCAConnector, HDMIConnect, PowerOutlet):
...
class DVDPlayer(HDMIConnect, PowerOutlet):
...
class GameConsole(HDMIConnect, EthernetConnector, PowerOutlet):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
